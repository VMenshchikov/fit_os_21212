...
void* swaper(void* arg) {
    ...
    if (rand() % 100 > 33) {
        if (storage->first != NULL) {
            pthread_rwlock_wrlock(&storage->first->sync);
            nodes[0] = storage->first;
            if (nodes[0]->next != NULL) {
                pthread_rwlock_wrlock(&nodes[0]->next->sync);
                ...
                pthread_rwlock_unlock(&storage->first->next->sync);
            }
    ...
    for (nodes[0] = storage->first; nodes[0]->next != NULL; nodes[0] = nodes[1]) {
        ...
        if (nodes[1]->next != NULL) {
            pthread_rwlock_wrlock(&nodes[1]->next->sync);
            nodes[2] = nodes[1]->next;
            ...
            pthread_rwlock_unlock(&nodes[2]->sync);
        }
        pthread_rwlock_unlock(&nodes[0]->sync);
    }
...
void* seeker1(void* arg) {
    ...
    for(;;) {
        ...
        for (Node* node = storage->first; node->next != NULL; node = node->next) {
            ...
            pthread_rwlock_rdlock(&node->next->sync);
