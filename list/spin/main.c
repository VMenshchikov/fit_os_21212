...
void* swaper(void* arg) {
    ...
    if (rand() % 100 > 33) {
        if (storage->first != NULL) {
            ...
            if (nodes[0]->next != NULL) {
                pthread_spin_lock(&nodes[0]->next->sync);
                ...
                pthread_spin_unlock(&storage->first->next->sync);
            }
            //pthread_spin_unlock(&nodes[0]->sync);
        }
    ...
    for (nodes[0] = storage->first; nodes[0]->next != NULL; nodes[0] = nodes[1]) {
        ...
        if (nodes[1]->next != NULL) {
            pthread_spin_lock(&nodes[1]->next->sync);
            nodes[2] = nodes[1]->next;
            ...
            pthread_spin_unlock(&nodes[2]->sync);
        }
        pthread_spin_unlock(&nodes[0]->sync);
    }
